...
import { Mic, MicOff, Pause, Play, X, AlertTriangle } from "lucide-react";
...
const Session = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setCurrentTurn((prev) => Math.min(prev + 1, mockTranscript.length));
    }, delay);
          ...
          >
            <div className="rounded-2xl border border-border bg-card p-8 shadow-lifted">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="font-display text-xl font-semibold text-foreground">Before we begin</h2>
              </div>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      {/* Session header */}
      <header className="border-b border-border bg-background/90 backdrop-blur-md px-6 py-4">
      ...
      <div className="flex-1 flex flex-col container mx-auto max-w-3xl px-6 py-8">
        {!sessionActive ? (
          ...
          >
            <motion.div
              className="h-24 w-24 rounded-full bg-sage-light flex items-center justify-center mb-8 cursor-pointer"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => setSessionActive(true)}
            >
              <div className="h-16 w-16 rounded-full bg-primary flex items-center justify-center animate-breathe">
                ...
                >
                  <div className="bg-card border border-border rounded-2xl rounded-bl-md px-5 py-3">
                    <div className="flex gap-1.5">
                      ...
                      <div className="h-2 w-2 rounded-full bg-muted-foreground/40 animate-pulse-soft" style={{ animationDelay: "0.3s" }} />
                      <div className="h-2 w-2 rounded-full bg-muted-foreground/40 animate-pulse-soft" style={{ animationDelay: "0.6s" }} />
                    </div>
                  </div>
                </motion.div>
              )}
